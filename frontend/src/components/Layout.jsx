...
const nav = [
  ...
  {
    to: "/app/medicine-price-ai",
    icon: DollarSign,
    label: "Medicine Price AI",
  },
  { to: "/app/simple-news", icon: Newspaper, label: "Healthcare News" },
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex">
      <aside
        className={`bg-slate-900 border-r border-slate-800 flex flex-col fixed h-full transition-all duration-300 ${collapsed ? "w-16" : "w-60"}`}
      >
        <div className="p-4 border-b border-slate-800 flex items-center gap-2">
          <NavLink to="/" className="flex items-center gap-2.5 flex-1">
        ...
        <div className="p-4 border-t border-slate-800">
          {!collapsed && (
            <p className="text-slate-500 text-xs truncate px-4 mb-2">
              {user?.email}
            </p>
          )}
          <button
            onClick={logout}
            className={`w-full ${collapsed ? "px-0 text-center" : "px-4 text-left"} py-2 text-rose-400 hover:bg-rose-500/10 rounded-xl text-sm font-medium transition-colors`}
          >
            Sign out
          </button>
        </div>
      ...
      <main
        className={`flex-1 p-8 overflow-auto transition-all duration-300 ${collapsed ? "ml-16" : "ml-60"}`}
      >
        <Outlet />
      </main>
    </div>
